...
public class NavigationTag extends TagSupport {
		...
		if (page == null) {
			...
			try {
				...
				if (page.getPage() > 1) {
					...
					writer.print("<li class=\"disabled\"><a herf=\"#\">" + "首页</a></li>");
				...
				for (int i = 1; i <= number && indexPage <= pageCount; indexPage++, i++) {
					if (indexPage == page.getPage()) {
						writer.print("<li class=\"active\"><a herf=\"#\">" + indexPage
								+ "<span class=\"sr-only\">(current)</span></a></li>");
						continue;
					}
					String pageUrl = append(url, "page", indexPage);
					pageUrl = append(pageUrl, "rows", page.getSize());
					writer.print("<li><a herf=\"" + pageUrl + "\">" + indexPage + "</a></li>");
				}
				// 显示“下一页”按钮
				if (page.getPage() < pageCount) {
					...
					writer.print("<li><a herf=\"" + backUrl + "\">" + "尾页</a></li>");
	...
	public void setNnumber(int number) {
		this.number = number;
